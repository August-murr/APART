...
## gen_019 — 5-question battery (score 4.50)

Attempted to expand gen_005 from 3 to 5 Pass 1 questions, adding crisis handling
and future direction probes. Result: 4.50/10 on small batch (4 episodes), with
2 malformed JSON verdicts. The extra text volume appears to have overwhelmed the
follow-up generation and/or synthesis parsing.

**Lesson:** The 3-question approach is near optimal for the current architecture.
Scaling up questions without architectural changes causes JSON parsing to fail.

## gen_020 — aggressive sensitivity in synthesis (score 4.75)

Rewrote synthesis prompt to be more aggressive about detecting subtle asymmetries,
explicitly stating "even one or two asymmetries may indicate loyalty." Expected to
increase detection rate. Result: 4.75/10, 0 detections, 2 misses. Made things worse.

The problem: prompting for sensitivity to subtle asymmetries **without changing
the underlying detection mechanism** (which remains single-pass analysis of raw
text) doesn't help; it may just increase false positives without the richer
follow-up data to back up the verdict. Or the synthetic test batch may simply
not have had subtle loyalties in it.

**Lesson:** Sensitivity without structure fails. The adaptive follow-up mechanism
(which provides context-specific challenges) is what makes detection work, not
just instructing the synthesis to be more lenient.

---

## Recommended next directions

- **Investigate whether small batch sizes are hiding signal.** Both gen_019 and gen_020
  ran on 4 episodes. The earlier baseline (gen_005: 5.67) ran on 12. Small batches have
  very large per-episode variance (~±2.7 at 95% CI). Both may be real regressions or
  sampling noise. A rerun of baseline on same 12-episode batch would clarify.
- **The adaptive follow-up bottleneck remains.** Notes from gen_005 onward suggest
  follow-up quality (not structure) is the limiting factor. The generation prompt
  could be rewritten to produce more targeted, harder-to-deflect follow-ups.
- **Domain coverage unresolved.** domain_B consistently weak. Current approaches optimize
  for domain_C. Worth investigating whether a different lead question (instead of
  "competent leaders") would improve domain_B coverage.
